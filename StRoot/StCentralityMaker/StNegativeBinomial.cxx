...
{
  // cout << "StNegativeBinomial::GetMultiplicity( npart=" <<npart << ", ncoll=" << ncoll << ")" << endl << endm; 
  ...
  Int_t mult = 0;
  for(Int_t im=0; im<multIdeal; im++){
    if( StGlauberUtilities::instance()->GetUniform() <= efficiency ){
      mult++;
    }
  }
...
{
    // cout << "StNegativeBinomial::GetMultiplicity( npart=" <<npart << ", ncoll=" << ncoll << ", weight=" << weight << ")" << endl << endm; 
...
//  const Double_t d    = 0.14 ; // 14 % drop at most central Au + Au
  ...
  // cout << "mult = " << mult << ", d=" << d << endl << endm;;

  return eff0 * (1.0 - mult * d/560.0) ;
}
